...
// open the database
  ...
  notlariGetir() async {
    var db = await _getDatabase();
    var sonuc = db.rawQuery('select * from "not" inner join kategori on kategori.kategoriID="not".kategoriID order by notID Desc;');
    return sonuc;
  }

  Future<List<Not>> notListesiniGetir() async {
    var notMapListesi = await notlariGetir();
    var notlistesi = List<Not>();
    for (Map map in notMapListesi) {
